...
HEADER = {
    "en": [
        ...
        ">   ```",
        ">",
        "> **Diagram** (optional): Add `### diagram` in any slide to draw a flowchart,",
        ">   AWS architecture, sequence/UML/ER, network, or mind map via the drawio skill.",
        ">   Requires the draw.io desktop CLI (`brew install --cask drawio`).",
        ">   `type:` (optional) one of: `flowchart` | `aws` | `sequence` | `class` | `er` | `network` | `mindmap`",
        ">",
        ">   Format:",
        ">   ```",
        ">   ### diagram",
        ...
        ">   Lambda calls Amazon Rekognition, then writes results to DynamoDB.",
        ">   ```",
    ],
    "zh": [
        "# 演示文稿标题",
        ...
        ">   ```",
        ">",
        "> **图表/架构图**（可选）：在任意幻灯片中添加 `### diagram`，可通过 drawio skill",
        ">   绘制流程图、AWS 架构图、时序/UML/ER 图、网络拓扑图或思维导图。",
        ">   需要安装 draw.io 桌面版 CLI（`brew install --cask drawio`）。",
        ">   `type:`（可选）取值：`flowchart`（流程图）| `aws`（架构图）| `sequence`（时序图）| `class`（类图）| `er`（ER 图）| `network`（网络图）| `mindmap`（思维导图）",
        ">",
        ">   格式：",
        ">   ```",
        ">   ### diagram",
        ...
        ">   用户将图片上传到 S3 存储桶。",
        ">   上传事件触发一个 Lambda 函数。",
        ">   Lambda 调用 Amazon Rekognition，然后将结果写入 DynamoDB。",
        ">   ```",
    ],
}
